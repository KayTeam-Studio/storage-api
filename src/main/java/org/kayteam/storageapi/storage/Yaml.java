...
public class Yaml {
    ...
    public void reloadYamlFile() {
                ...
                if (file.createNewFile()) {
                    if (javaPlugin != null) {
                        ...
                        if (!directory.equals(javaPlugin.getDataFolder().getPath())) {
                            localDirectory = directory.replaceAll(javaPlugin.getDataFolder().getPath(), "");
        ...
        if (contains("replacements.global") && fileConfiguration.isConfigurationSection("replacements.global")) {
        ...
        if (contains("replacements.local") && fileConfiguration.isConfigurationSection("replacements.local")) {
    ...
    public FileConfiguration getFileConfiguration() {
        if (fileConfiguration == null) reloadYamlFile();
    ...
    public void generateBackup() {
        ...
        backup.saveWithOtherFileConfiguration(fileConfiguration);
    }
    ...
    public static List<Yaml> getYamlFiles(String directory) {
        ...
        if (dir.exists()) {
            ...
            if (files != null) for (File file:files) simpleYamlList.add(new Yaml(directory, file.getName().replaceAll(".yml", "")));
        }
    ...
    public ConfigurationSection getConfigurationSection(String path) {
        if (fileConfiguration.contains(path) && isConfigurationSection(path)) return fileConfiguration.getConfigurationSection(path);
        return null;
    }
    ...
    public Object get(String path) {
        if (fileConfiguration.contains(path)) return fileConfiguration.get(path);
        return null;
    }
    ...
    public boolean getBoolean(String path) {
        if (contains(path) && isBoolean(path)) return fileConfiguration.getBoolean(path);
        return false;
    }
    ...
    public int getInt(String path) {
        if (contains(path) && isInt(path)) return fileConfiguration.getInt(path);
        return -1;
    }
    ...
    public long getLong(String path) {
        if (contains(path) && isLong(path)) return fileConfiguration.getLong(path);
        return -1;
    }
    ...
    public double getDouble(String path) {
        if (contains(path) && isDouble(path)) return fileConfiguration.getDouble(path);
        return -1.0;
    }
    ...
    public String getString(String path) {
        if (contains(path) && isString(path)) return fileConfiguration.getString(path);
        return "";
    }
    ...
    public String getString(String path, String def,String[][] replacements) {
        ...
        for (String[] replacement:replacements) string = string.replaceAll(replacement[0], replacement[1]);
        return string;
    }

    public boolean isStringList(String path) {
        if (contains(path)) return fileConfiguration.getList(path).get(0).getClass().getSimpleName().equals("String");
        return false;
    }

    public List<String> getStringList(String path) {
        if (contains(path) && isList(path)) return fileConfiguration.getStringList(path);
    ...
    public List<?> getList(String path) {
        if (contains(path) && isList(path)) return fileConfiguration.getList(path);
        return new ArrayList<>();
    }
    ...
    public void sendMessage(CommandSender commandSender, String path, String[][] replacements) {
        ...
        for (String message : messages) {
            ...
            for (String text : Yaml.globalReplacements.keySet()) {
                message = message.replaceAll("%" + text + "%", Yaml.globalReplacements.get(text));
            }
            // PlaceholderAPI
    ...
    public static void sendSimpleMessage(CommandSender commandSender, Object message, String[][] replacements) {
        ...
        for (String m : messages) {
            ...
            // Global replacements
            for (String text : Yaml.globalReplacements.keySet()) {
                m = m.replaceAll("%" + text + "%", Yaml.globalReplacements.get(text));
            }
            // PlaceholderAPI
            if (commandSender instanceof Player && Bukkit.getPluginManager().getPlugin("PlaceholderAPI") != null) {
